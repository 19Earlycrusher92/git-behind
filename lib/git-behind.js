...
const svg = ' \
<style> \
	#git-behind-svg:hover { fill: #587eff; } \
</style> \
<svg id="git-behind-svg" style="cursor: pointer; padding-top: 4px; margin-right: -4px;" width="20px" height="24px" fill="currentColor" viewBox="0 0 96 96" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid"> \
<g id="XMLID_1_"> \
...
module.exports = {
  ...
  consumeStatusBar(statusBar) {
    ...
    this.statusIcon.onclick = () => { atom.workspace.open("atom://config/packages/git-behind") }
  ...
  refresh() {
    ...
    icon.setAttribute('fill','currentColor')
		...
		setTimeout(() => { // Prepare yourself for 5 layers of callback spaghetti
	    // Check if folder exsist
	    let paths = atom.project.getPaths()
	    if (!paths || !paths[0]) return that.error('No project', 'Git Behind: No project open')
	    let path = 'cd '+paths[0]+' && '

	    // Check it's a git repo
	    let cmd = '[ -d .git ] || git rev-parse --git-dir'
	    exec(path+cmd, (error, stdout, stderr) => {
